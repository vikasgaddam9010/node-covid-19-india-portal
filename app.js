  ...
  try {
    ...
    if (user === undefined) {
      ...
      if (!ispasswordright) {
        ...
        const jwtToken = jwttoken.sign(payload, 'secretkey')
        res.send({jwtToken})
      }
    }
  } catch (e) {
...
app.get('/states', check, async (req, res) => {
  try {
    ...
    res.send(ans.map(each => convertCamelCase(each)))
  } catch (e) {
...
const convertDistrictsTOCamelCase = each => {
  return {
...
app.get('/districts/:districtId/', check, async (req, res) => {
  ...
  res.send(convertDistrictsTOCamelCase(ans))
})
